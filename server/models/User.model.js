...
UserSchema.methods.updateShow = function updateShow({ showId, fields }, cb) {
  const showToUpdate = this.trackedShows.find(
    (trackedShow) => trackedShow._id === Number(showId)
  );
  ...
  Object.entries(updateableFields).forEach(([key, value]) => {
    if (value !== undefined) showToUpdate[key] = value;
  });
  this.save(cb);
};
...
UserSchema.methods.toggleEpisodes = function (
  ...
  if (markAs === 'seen') {
    ...
    show.seenEpisodeIds = show.seenEpisodeIds.filter(
...
UserSchema.methods.getPublic = function getPublic() {
  ...
  const showsMap = shows.reduce(
    (acc, show) => Object.assign({}, acc, { [show.id]: show }),
    {}
  );
